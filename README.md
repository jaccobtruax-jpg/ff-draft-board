...
- **🧭 Draft Board page (Skeleton Key format)** — a third mode mirroring the UDK's "Jason's Skeleton Key Doc" (public Google Sheet we pulled from thefantasyfootballers.com UDK 2026 Cheatsheets): snake grid (rounds × teams, pre-numbered cells), a vertical master list of every pick (pick # | team | player | pos), and per-team "League Rosters" panels with pick numbers. Better than the sheet: it fills **live from the draft log**, shows keepers (K row + K tags), highlights your column, and color-codes positions.
- `test_harness.html` — in-browser smoke test, **120 checks** (open it, PASS/FAIL list renders)
