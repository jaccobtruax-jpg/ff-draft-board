...
- **🎛 Draft Command (right panel)** — the two old right columns merged into one panel with a **view dropdown**: 🎯 Next Pick Call (now with read detail: sim count, window, baseline mode + pts) · 👥 My Roster · 📉 Scarcity & Runs (new runs/snipers pressure box) · 📜 Draft Log · ⭐ My Guys · 📚 Reference (injury/UDK notes/expert lists). Choice persists. No more stacked redundancy.
- `test_harness.html` — in-browser smoke test, **130 checks** (open it, PASS/FAIL list renders)
