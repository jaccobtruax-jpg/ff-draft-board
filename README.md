...
- **★ My Guys presets** — one click loads a host's "My Guys" from the show as your starred list (Mike / Jason / Andy; dropdown in the board toggles). Data lives in `data/myguys.json` (names resolved to player IDs, re-applied on load, so a data refresh swaps in the latest episode's picks). Currently populated from the 2025 My Guys episode (Ep. 1783); a watcher cron pings the podcast feed the moment the 2026 episode airs.
- `test_harness.html` — in-browser smoke test, **110 checks** (open it, PASS/FAIL list renders)
